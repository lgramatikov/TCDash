...
TCDashboardInterfaceWatcher::TCDashboardInterfaceWatcher(AllJoynBusAttachment^ busAttachment) :
...
TCDashboardInterfaceWatcher::~TCDashboardInterfaceWatcher()
{
    UnregisterFromBus();
}

void TCDashboardInterfaceWatcher::UnregisterFromBus()
{
    if (nullptr != m_aboutListener)
    {
        PCSTR interfaces[] = { "org.laz.TCDashboardInterface" };
...
void TCDashboardInterfaceWatcher::OnAnnounce(
    _In_ PCSTR name,
    ...
    _In_ const alljoyn_msgarg aboutDataArg)
{
    ...
    if (alljoyn_aboutobjectdescription_hasinterface(objectDescription, "org.laz.TCDashboardInterface"))
    {
        AllJoynServiceInfo^ args = ref new AllJoynServiceInfo(
            AllJoynHelpers::MultibyteToPlatformString(name),
            AllJoynHelpers::GetObjectPath(objectDescription, "org.laz.TCDashboardInterface"),
            port);
...
void TCDashboardInterfaceWatcher::BusAttachmentStateChanged(_In_ AllJoynBusAttachment^ sender, _In_ AllJoynBusAttachmentStateChangedEventArgs^ args)
{
    if (args->State == AllJoynBusAttachmentState::Connected)
    {
        alljoyn_aboutlistener_callback callbacks = 
        {
            AllJoynHelpers::AnnounceHandler<TCDashboardInterfaceWatcher>
        };
...
void TCDashboardInterfaceWatcher::Start()
{
    ...
    int32 result = AllJoynHelpers::CreateInterfaces(m_busAttachment, c_TCDashboardInterfaceIntrospectionXml);
    ...
    m_busAttachmentStateChangedToken = m_busAttachment->StateChanged += ref new TypedEventHandler<AllJoynBusAttachment^, AllJoynBusAttachmentStateChangedEventArgs^>(this, &TCDashboardInterfaceWatcher::BusAttachmentStateChanged);
    m_busAttachment->Connect();
}

void TCDashboardInterfaceWatcher::Stop()
{
    StopInternal(AllJoynStatus::Ok);
}

void TCDashboardInterfaceWatcher::StopInternal(int32 status)
{
    UnregisterFromBus();
    AllJoynHelpers::DispatchEvent([=]() {
